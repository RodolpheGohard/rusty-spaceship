...
(function (exports) {
	...
	MotorSound.prototype.process = function (event) {
		...
		var channelLeft = event.outputBuffer.getChannelData(0);
		var channelRight = event.outputBuffer.getChannelData(1);
		var index;

		for (var i = 0; i < channelRight.length; ++i) {
			// skip more data frames on higher speed
			this.currentFrame += this.speed;
			index = Math.floor(this.currentFrame) % this.data.length;
			// update buffer from data
			channelRight[i] = this.data[index];
			channelLeft[i] = this.data[index];
		}
